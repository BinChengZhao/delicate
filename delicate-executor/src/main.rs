...
async fn create_task(Json(signed_task_package): Json<SignedTaskPackage>,
...
pub async fn pre_create_task(signed_task_package: SignedTaskPackage,
                             shared_delay_timer: Data<&Arc<DelayTimer>>,
                             executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                             -> Result<(), CommonError> {
...
#[handler]
#[instrument(skip(executor_conf, shared_delay_timer, signed_task_package), fields(task_package = signed_task_package.task_package.id))]
async fn update_task(Json(signed_task_package): Json<SignedTaskPackage>,
                     shared_delay_timer: Data<&Arc<DelayTimer>>,
                     executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                     -> Json<UnitUnifiedResponseMessages> {
    let response: UnitUnifiedResponseMessages =
...
pub async fn pre_update_task(signed_task_package: SignedTaskPackage,
                             shared_delay_timer: Data<&Arc<DelayTimer>>,
                             executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                             -> Result<(), CommonError> {
...
async fn remove_task(Json(signed_task_unit): Json<SignedTaskUnit>,
                     shared_delay_timer: Data<&Arc<DelayTimer>>,
                     executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                     -> Json<UnitUnifiedResponseMessages> {
    let response: UnitUnifiedResponseMessages =
        pre_remove_task(signed_task_unit, shared_delay_timer, executor_conf).await.into();
    Json(response)
}

pub async fn pre_remove_task(signed_task_unit: SignedTaskUnit,
                             shared_delay_timer: Data<&Arc<DelayTimer>>,
                             executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                             -> Result<(), CommonError> {
...
#[handler]
#[instrument(skip(executor_conf, shared_delay_timer, signed_task_unit), fields(task_id = signed_task_unit.task_unit.task_id))]
async fn advance_task(Json(signed_task_unit): Json<SignedTaskUnit>,
                      shared_delay_timer: Data<&Arc<DelayTimer>>,
                      executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                      -> Json<UnitUnifiedResponseMessages> {
    let response: UnitUnifiedResponseMessages =
        pre_advance_task(signed_task_unit, shared_delay_timer, executor_conf).await.into();
    Json(response)
}

pub async fn pre_advance_task(signed_task_unit: SignedTaskUnit,
                              shared_delay_timer: Data<&Arc<DelayTimer>>,
                              executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                              -> Result<(), CommonError> {
...
async fn cancel_task(Json(signed_cancel_task_record): Json<SignedCancelTaskRecord>,
                     shared_delay_timer: Data<&Arc<DelayTimer>>,
                     executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                     -> Json<UnitUnifiedResponseMessages> {
    let response: UnitUnifiedResponseMessages =
        pre_cancel_task(signed_cancel_task_record, shared_delay_timer, executor_conf).await.into();
    Json(response)
}

pub async fn pre_cancel_task(signed_cancel_task_record: SignedCancelTaskRecord,
                             shared_delay_timer: Data<&Arc<DelayTimer>>,
                             executor_conf: Data<&Arc<ExecutorSecurityConf>>)
                             -> Result<(), CommonError> {
...
#[allow(dead_code)]
async fn maintenance(shared_delay_timer: Data<&Arc<DelayTimer>>)
                     -> Json<UnitUnifiedResponseMessages> {
    Json(Into::<UnitUnifiedResponseMessages>::into(shared_delay_timer.stop_delay_timer()))
}
...
async fn health_screen(req: &Request,
...
// Or set security level, no authentication at level 0, public and private keys
// required at level 1.
async fn bind_executor(Json(request_bind_scheduler): Json<SignedBindRequest>,
    ...
    if verify_result.is_ok() {
        ...
        // Take 10 bits from executor_machine_id and do machine_id and node_id in two
        // groups.
...
fn main() -> AnyResult<()> {
    ...
    let block_result: AnyResult<()> =
        arc_runtime.block_on(async move {
                       let route =
                           Route::new().at("/api/task/update", post(update_task))
...
async fn init_executor(app: Route, arc_runtime: Arc<Runtime>) -> impl Endpoint {
    let mut delay_timer = DelayTimerBuilder::default().tokio_runtime_shared_by_custom(arc_runtime)
...
fn launch_status_reporter(delay_timer: &mut DelayTimer,
        ...
        tokio_spawn(async move {
            ...
            loop {
                let f = async {
                    ...
                    {
                        send_event_collection(scheduler.as_ref(),
...
async fn fresh_scheduler_conf(shared_security_conf: &ExecutorSecurityConf,
...

    {
        ...
        if fresh_scheduler_time != scheduler_time {
            ...
            // Adjust the internal host to avoid the need to clone String when calling
            // RequestClient::post.
...
async fn collect_events(status_reporter: &StatusReporter,
    ...
    for _i in 0..10 {
        let event_future: TokioTimeout<_> =
            tokio_timeout(Duration::from_secs(3),
...
async fn send_event_collection(scheduler: Option<&BindRequest>,
                               executor_event_collection: SignedExecutorEventCollection,
                               client: &RequestClient) {
    if let Some(scheduler_ref) = scheduler.as_ref() {
        debug!("Event collection - {:?}", &executor_event_collection.event_collection);

        if let Ok(response) =
            client.post(&scheduler_ref.scheduler_host)
                  .json(&executor_event_collection)
                  .send()
                  .await
                  .map_err(|e| {
                      error!("Failed to send the event collection: {} - {} - {:?}",
                             e, &scheduler_ref, &executor_event_collection.event_collection)
                  })
        {
            response.bytes().await.map(|b| debug!("delicate-schduler response: {:?}", b)).ok();
        }
    }
}
