...
impl SecurityLevel {
    ...
    pub fn app_security_level() -> Self {
        env::var_os("DELICATE_SECURITY_LEVEL").map_or(SecurityLevel::default(), |e| {
                ...
                .and_then(|e| e.and_then(|e| e.try_into().ok()))
...
pub fn make_signature<T: Serialize>(data: &T,
                                    token: Option<&str>)
...
pub fn verify_signature_by_raw_data<T: Serialize>(data: &T,
                                                  token: Option<&str>,
                                                  signature: &[u8])
                                                  -> Result<(), crate::error::CommonError> {
    let signature_new = make_signature(data, token)?;
    if signature_new.eq(signature) { Ok(()) } else { Err(crate::error::CommonError::DisVerify) }
}
