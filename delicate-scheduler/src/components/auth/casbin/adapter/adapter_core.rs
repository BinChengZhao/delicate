...
use crate::{db::model::casbin_rule::*, prelude::*};
...
pub(crate) fn load_filtered_policy_line(casbin_rule: &CasbinRule,
                                        f: &Filter<'_>)
...
fn normalize_policy(casbin_rule: &CasbinRule) -> Option<Vec<String>> {
    let mut auth_service_result = vec![&casbin_rule.v0,
                                       &casbin_rule.v1,
                                       &casbin_rule.v2,
                                       &casbin_rule.v3,
                                       &casbin_rule.v4,
...
impl Adapter for DieselAdapter {
    async fn load_policy(&self, m: &mut dyn Model) -> Result<(), CasbinError> {
        let conn = self.pool.get().map_err(|e| casbin::error::AdapterError(Box::new(e)))?;

        let rules = spawn_blocking::<_, Result<_, AuthServiceError>>(move || {
                        adapter::load_policy(conn)
                    }).await
    ...
    async fn clear_policy(&mut self) -> Result<(), CasbinError> {
        ...
        Ok(spawn_blocking::<_, Result<_, AuthServiceError>>(move || adapter::clear_policy(conn))
            .await
            .map_err(|e| casbin::error::AdapterError(Box::new(e)))?
            .map_err(|e| AdapterError(Box::new(e)))?)
    }

    async fn load_filtered_policy<'a>(&mut self,
                                      m: &mut dyn Model,
                                      f: Filter<'a>)
                                      -> Result<(), CasbinError> {
        let conn = self.pool.get().map_err(|e| casbin::error::AdapterError(Box::new(e)))?;
    ...
    async fn add_policies(&mut self,
                          _sec: &str,
                          ptype: &str,
    ...
    async fn remove_policy(&mut self,
                           _sec: &str,
                           pt: &str,
                           rule: Vec<String>)
                           -> Result<bool, CasbinError> {
    ...
    async fn remove_policies(&mut self,
                             _sec: &str,
                             pt: &str,
                             rules: Vec<Vec<String>>)
                             -> Result<bool, CasbinError> {
    ...
    async fn remove_filtered_policy(&mut self,
                                    _sec: &str,
                                    pt: &str,
                                    field_index: usize,
...
mod tests {
    ...
    async fn test_adapter() {
        ...
        assert!(adapter.remove_policy("", "p", to_owned(vec!["alice", "data1", "read"]))
                       .await
                       .is_ok());
        assert!(adapter.remove_policy("", "p", to_owned(vec!["bob", "data2", "write"]))
                       .await
                       .is_ok());
        assert!(adapter.remove_policy("", "p", to_owned(vec!["data2_admin", "data2", "read"]))
                       .await
                       .is_ok());
        assert!(adapter.remove_policy("", "p", to_owned(vec!["data2_admin", "data2", "write"]))
                       .await
                       .is_ok());
        assert!(adapter.remove_policy("", "g", to_owned(vec!["alice", "data2_admin"]))
                       .await
                       .is_ok());

        assert!(adapter.add_policy("", "p", to_owned(vec!["alice", "data1", "read"]))
                       .await
                       .is_ok());
        assert!(adapter.add_policy("", "p", to_owned(vec!["bob", "data2", "write"])).await.is_ok());
        assert!(adapter.add_policy("", "p", to_owned(vec!["data2_admin", "data2", "read"]))
                       .await
                       .is_ok());
        assert!(adapter.add_policy("", "p", to_owned(vec!["data2_admin", "data2", "write"]))
                       .await
                       .is_ok());

        assert!(adapter.remove_policies("",
                                        "p",
                                        vec![to_owned(vec!["alice", "data1", "read"]),
        ...
        assert!(adapter.add_policy("", "g", to_owned(vec!["alice", "data2_admin"])).await.is_ok());
        ...
        assert!(adapter.add_policy("", "g", to_owned(vec!["alice", "data2_admin"])).await.is_ok());
        assert!(adapter.add_policy("", "g", to_owned(vec!["alice", "data2_admin"])).await.is_err());
        ...
        assert!(adapter.remove_filtered_policy("", "g", 0, to_owned(vec!["alice", "data2_admin"]))
                       .await
                       .is_ok());
        ...
        let filter = Filter { p: vec!["", "domain1"], g: vec!["", "", "domain1"] };
