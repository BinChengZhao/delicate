...
use crate::{
    db::{model::casbin_rule::*, schema},
    prelude::*,
};

pub fn remove_policy(conn: db::PoolConnection,
                     pt: &str,
                     rule: Vec<String>)
    ...
    let filter = ptype.eq(pt)
                      ...
                      .and(v4.eq(&rule[4]))
                      .and(v5.eq(&rule[5]));
    Ok(diesel::delete(casbin_rule.filter(filter)).execute(&conn).map(|n| n == 1)?)
}

pub fn remove_policies(conn: db::PoolConnection,
                       pt: &str,
...
pub fn remove_filtered_policy(conn: db::PoolConnection,
                              pt: &str,
                              field_index: usize,
    ...
    let boxed_query = if field_index == 5 {
        ...
        diesel::delete(casbin_rule.filter(ptype.eq(pt)
                                               ...
                                               .and(eq_empty!(&field_values[4], v5)))).into_boxed()
    } else {
        diesel::delete(casbin_rule.filter(ptype.eq(pt)
                                               ...
                                               .and(eq_empty!(&field_values[5], v5)))).into_boxed()
    };
...
pub(crate) fn save_policy(conn: db::PoolConnection,
    ...
    Ok(conn.transaction::<_, DieselError, _>(|| {
               ...
               Ok(diesel::insert_into(casbin_rule).values(&rules)
                                                  .execute(&*conn)
                                                  .and_then(|n| {
                                                      if n == rules.len() {
                                                          Ok(())
...
pub(crate) fn add_policy(conn: db::PoolConnection,
                         new_rule: NewCasbinRule)
                         -> AuthServiceResult<bool> {
    use schema::casbin_rule::dsl::casbin_rule;

    Ok(diesel::insert_into(casbin_rule).values(&new_rule).execute(&conn).map(|n| n == 1)?)
}

pub(crate) fn add_policies(conn: db::PoolConnection,
                           new_rules: Vec<NewCasbinRule>)
                           -> AuthServiceResult<bool> {
    use schema::casbin_rule::dsl::casbin_rule;

    Ok(conn.transaction::<_, DieselError, _>(|| {
               diesel::insert_into(casbin_rule).values(&new_rules)
                                               .execute(&*conn)
                                               .and_then(|n| {
                                                   if n == new_rules.len() {
                                                       Ok(true)
