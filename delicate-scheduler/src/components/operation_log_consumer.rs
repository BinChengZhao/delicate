...
lazy_static! {
    pub static ref OPERATION_LOG_CONSUMERS: (AsyncSender<NewOperationLogPair>, AsyncReceiver<NewOperationLogPair>) =
...
pub(crate) async fn loop_operate_logs(pool: Arc<db::ConnectionPool>) {
    loop {
        ...
        if let Ok(conn) = pool.get() {
            operate_logs(conn, operation_log_pairs).await
...
pub(crate) async fn send_option_operation_log_pair(option_operation_log_pair: NewOperationLogPairOption)
{
...
async fn operate_logs(conn: db::PoolConnection,
                      logs: Vec<NewOperationLogPair>)
                      -> Result<(), CommonError> {
    let mut operation_logs = Vec::new();
    let mut operation_log_details = Vec::new();

    logs.into_iter().for_each(|(operation_log, operation_log_detail)| {
    ...
    spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
        // Use this solution to ensure that the conditions are met
        // (innodb_autoinc_lock_mode = 1 ("executive" lock mode)): https://stackoverflow.com/questions/27225804/mysql-batch-insert-on-multiple-tables-with-last-insert-id
        // https://dev.mysql.com/doc/refman/8.0/en/innodb-auto-increment-handling.html
        // https://dev.mysql.com/doc/refman/5.6/en/information-functions.html#function_last-insert-id

        diesel::insert_into(operation_log::table).values(&operation_logs).execute(&conn)?;

        let last_id = diesel::select(db::last_insert_id).get_result::<u64>(&conn)?;

        operation_log_details.iter_mut()
        ...
        diesel::insert_into(operation_log_detail::table).values(&operation_log_details)
                                                        .execute(&conn)?;
        Ok(())
    }).await??;
