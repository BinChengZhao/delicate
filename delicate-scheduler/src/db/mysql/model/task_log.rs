...
impl QueryParamsTaskLog {
    ...
    pub(crate) fn query_filter<ST>(self, mut statement_builder : task_log::BoxedQuery<'static, Mysql, ST>) -> task_log::BoxedQuery<'static, Mysql, ST> {
        statement_builder = statement_builder
            .filter(task_log::status.ne(2));
        ...
        if let Some(task_description) = self.description {
            statement_builder = statement_builder.filter(task_log::description.like(task_description));
        }
