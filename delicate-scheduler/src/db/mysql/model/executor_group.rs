...
impl ExecutorGroupQueryBuilder{
    ...
    pub(crate) fn query_count()-> executor_group::BoxedQuery<'static, Mysql, diesel::sql_types::Bigint>{
        executor_group::table
        .into_boxed()
        .count()
    }
...
impl QueryParamsExecutorGroup {
    ...
    pub(crate) fn query_filter<ST>(self, mut statement_builder : executor_group::BoxedQuery<'static, Mysql, ST>) -> executor_group::BoxedQuery<'static, Mysql, ST> {
        statement_builder = statement_builder
            .filter(executor_group::status.ne(2));
        ...
        if let Some(executor_group_name) = self.name {
            statement_builder = statement_builder.filter(executor_group::name.like(executor_group_name));
        }

        if let Some(executor_group_description) = self.description {
            statement_builder = statement_builder.filter(executor_group::description.like(executor_group_description));
        }

        if let Some(executor_group_tag) = self.tag {
            statement_builder = statement_builder.filter(executor_group::tag.like(executor_group_tag));
        }
