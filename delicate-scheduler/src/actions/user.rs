...
async fn create_user(req: &Request,
    ...
    if let Ok(conn) = pool.get() {
        let f_result =
            spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                conn.transaction(|| {
                        diesel::insert_into(user::table).values(&new_user).execute(&conn)?;
                        ...
                        diesel::insert_into(user_auth::table).values(&user_auths.0[..])
                                                             .execute(&conn)?;
...
async fn show_users(Json(query_params): Json<model::QueryParamsUser>,
                    pool: Data<&Arc<db::ConnectionPool>>)
                    -> impl IntoResponse {
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           ...
                           Ok(PaginateData::<model::User>::default().set_data_source(users)
...
async fn update_user(req: &Request,
...
async fn change_password(req: &Request,
    ...
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                ...
                .set(user_auth::certificate.eq(get_encrypted_certificate_by_raw_certificate(
...
async fn login_user(req: &Request,
...
async fn pre_login_user(req: &Request,
                        model::UserAuthLogin { login_type, account, password }: model::UserAuthLogin,
                        pool: Data<&Arc<db::ConnectionPool>>)
                        -> Result<(), CommonError> {
    let client_ip = req.remote_addr().as_socket_addr().map(|sock| sock.ip().to_string());
    let mut new_user_login_log = NewUserLoginLog::default();
    new_user_login_log.set_lastip(client_ip).set_login_type(login_type);
    ...
    let user_package: (model::UserAuth, model::User) =
        spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
            ...
            .filter(
                user_auth::certificate.eq(get_encrypted_certificate_by_raw_certificate(&password)),
            )
            ...
            diesel::insert_into(user_login_log::table).values(&new_user_login_log)
                                                      .execute(&conn)
                                                      .ok();
...
async fn check_user(req: &Request, pool: Data<&Arc<db::ConnectionPool>>) -> impl IntoResponse {
    ...
    if let Ok(user) = check_result {
        return Json(UnifiedResponseMessages::<model::User>::success_with_data(user))
            .into_response();
    };
...
async fn pre_check_user(req: &Request,
    ...
    let user = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                   let user = user::table.select(user::all_columns)
                                         .find(user_id)
...
async fn roles(enforcer: Data<&Arc<AsyncRwLock<Enforcer>>>,
    ...
    Json(UnifiedResponseMessages::<Vec<String>>::success_with_data(roles))
}

#[handler]

async fn permissions(enforcer: Data<&Arc<AsyncRwLock<Enforcer>>>,
                     Json(UserName { user_name }): Json<UserName>)
                     -> impl IntoResponse {
    let mut enforcer_guard = enforcer.write().await;
...
async fn append_role(req: &Request,
    ...
    operate_roles.dedup();
    let append_roles: Vec<String> =
        operate_roles.iter()
                     .filter_map(|role_id| {
                         ROLES.get(*role_id).map(|role_name| role_name.to_string())
                     })
                     .collect();
    ...
    let mut enforcer_guard = enforcer.write().await;
    let operated_result = enforcer_guard.add_roles_for_user(&user_name, append_roles, None).await;
...
async fn delete_role(req: &Request,
                     enforcer: Data<&Arc<AsyncRwLock<Enforcer>>>,
                     Json(user_and_roles): Json<UserAndRoles>)
                     -> impl IntoResponse {
    let operation_log_pair_option =
    ...
    let delete_roles: Vec<&str> =
        operate_roles.iter().filter_map(|role_id| ROLES.get(*role_id).copied()).collect();
    ...
    for role in delete_roles {
        enforcer_guard.delete_role_for_user(&user_name, role, None)
                      .await
...
async fn append_permission(req: &Request,
                           enforcer: Data<&Arc<AsyncRwLock<Enforcer>>>,
    ...
    let mut enforcer_guard = enforcer.write().await;
    let operated_result =
        enforcer_guard.add_permissions_for_user(&user_name, operate_permissions).await;
    let msg = Into::<UnifiedResponseMessages<bool>>::into(operated_result);
    Json(msg)
}

#[handler]
async fn delete_permission(req: &Request,
                           enforcer: Data<&Arc<AsyncRwLock<Enforcer>>>,
                           Json(user_and_permissions): Json<UserAndPermissions>)
                           -> impl IntoResponse {
    let operation_log_pair_option =
    ...
    for operate_permission in operate_permissions {
        enforcer_guard.delete_permission_for_user(&user_name, operate_permission)
                      .await
