...
pub(crate) fn route_config() -> Route {
    Route::new().at("/api/executor_group/list", post(show_executor_groups))
                .at("/api/executor_group/detail", post(show_executor_group_detail))
...
#[handler]
async fn create_executor_group(req: &Request,
    ...
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           diesel::insert_into(executor_group::table).values(&executor_group)
                                                                     .execute(&conn)?;
...
#[handler]
async fn show_executor_groups(Json(query_params): Json<model::QueryParamsExecutorGroup>,
                              pool: Data<&Arc<db::ConnectionPool>>)
                              -> impl IntoResponse {
    if let Ok(conn) = pool.get() {
        let f_result =
            spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                ...
                let executor_groups = query_params.clone()
                ...
                Ok(PaginateData::<model::ExecutorGroup>::default().set_data_source(executor_groups)
...
#[handler]
async fn show_executor_group_detail(Json(model::ExecutorGroupId { executor_group_id }): Json<model::ExecutorGroupId>,
                                    pool: Data<&Arc<db::ConnectionPool>>)
                                    -> impl IntoResponse {
    ...
    if let Ok(executor_group_detail) = executor_group_detail_result {
        return Json(UnifiedResponseMessages::<model::ExecutorGroupDetail>::success_with_data(
...
async fn pre_show_executor_group_detail(executor_group_id: i64,
                                        pool: Data<&Arc<db::ConnectionPool>>)
    ...
    let executor_group_detail: model::ExecutorGroupDetail =
        spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
            let executor_group_detail_inner =
                executor_group::table.select(executor_group::all_columns)
            ...
            Ok(model::ExecutorGroupDetail { inner: executor_group_detail_inner, bindings })
        }).await??;
...
#[handler]
async fn update_executor_group(req: &Request,
    ...
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           diesel::update(&executor_group).set(&executor_group).execute(&conn)
                       }).await;
...
#[handler]
async fn delete_executor_group(req: &Request,
    ...
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           // Cannot link to delete internal bindings, otherwise it will cause data
                           // misalignment.
