...
pub(crate) fn route_config() -> Route {
    Route::new().at("/api/operation_log/list", post(show_operation_log))
                .at("/api/operation_log/detail", post(show_operation_log_detail))
}

#[handler]

async fn show_operation_log(Json(query_params): Json<model::QueryParamsOperationLog>,
    ...
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           ...
                           let operation_log = query_params.clone()
    ...
    Json(UnifiedResponseMessages::<PaginateData<model::FrontEndOperationLog>>::error())
}
#[handler]

async fn show_operation_log_detail(Json(query_params): Json<model::OperationLogId>,
                                   pool: Data<&Arc<db::ConnectionPool>>)
                                   -> impl IntoResponse {
    ...
    if let Ok(conn) = pool.get() {
        ...
        let log_detail = f_result.map(|log_detail_result| {
