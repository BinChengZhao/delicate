    ...
    if let Ok(conn) = pool.get() {
        let daily_state_result =
            spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                ...
                create_count.iter_mut().for_each(|s| s.status = State::Running as i16);
                ...
                Ok(pre_show_one_day_tasks_state(hours_range, create_count, end_states_count))
            }).await;
        let daily_state =
            daily_state_result.map(|daily_state_result| {
...
pub(crate) fn pre_show_one_day_tasks_state(hours_range: Vec<u32>,
    ...
    let mut created_map: HashMap<u32, i64> =
        create_count.iter().map(|t| (t.hour_num as u32, t.total)).collect();
    ...
    hours_range.iter().for_each(|h| {
                          daily_states.created.push(created_map.remove(h).unwrap_or(0));
                          daily_states.timeout.push(timeout_map.remove(h).unwrap_or(0));
                          daily_states.finished.push(finished_map.remove(h).unwrap_or(0));
                          daily_states.abnormal.push(abnormal_map.remove(h).unwrap_or(0));
                          daily_states.canceled.push(canceled_map.remove(h).unwrap_or(0));
