...
pub(crate) fn route_config() -> Route {
    Route::new().at("/api/executor_processor/list", post(show_executor_processors))
                .at("/api/executor_processor/create", post(create_executor_processor))
                .at("/api/executor_processor/update", post(update_executor_processor))
                .at("/api/executor_processor/delete", post(delete_executor_processor))
                .at("/api/executor_processor/activate", post(activate_executor_processor))
}

#[handler]
async fn create_executor_processor(req: &Request,
...
#[handler]
async fn show_executor_processors(Json(query_params): Json<model::QueryParamsExecutorProcessor>,
                                  pool: Data<&Arc<db::ConnectionPool>>)
                                  -> impl IntoResponse {
    if let Ok(conn) = pool.get() {
        let f_result =
            spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                ...
                let executor_processors = query_params.clone()
    ...
    Json(UnifiedResponseMessages::<PaginateData<model::ExecutorProcessor>>::error())
}

#[handler]
async fn update_executor_processor(req: &Request,
    ...
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           diesel::update(&executor_processor).set(&executor_processor)
                                                              .execute(&conn)
                       }).await;
...
#[handler]
async fn delete_executor_processor(req: &Request,
                                   Json(model::ExecutorProcessorId { executor_processor_id }): Json<model::ExecutorProcessorId>,
                                   pool: Data<&Arc<db::ConnectionPool>>)
                                   -> impl IntoResponse {
...
#[handler]
async fn activate_executor_processor(req: &Request,
                                     Json(model::ExecutorProcessorId { executor_processor_id }): Json<model::ExecutorProcessorId>,
                                     pool: Data<&Arc<db::ConnectionPool>>,
                                     scheduler: Data<&Arc<SchedulerMetaInfo>>)
                                     -> impl IntoResponse {
    let uniform_data: UnifiedResponseMessages<()> =
        do_activate(req, pool, executor_processor_id, scheduler).await.into();
    Json(uniform_data)
}
async fn do_activate(req: &Request,
                     pool: Data<&Arc<db::ConnectionPool>>,
...
async fn activate_executor(req: &Request,
                           conn: db::PoolConnection,
                           executor_processor_id: i64,
                           scheduler: Data<&Arc<SchedulerMetaInfo>>)
                           -> Result<service_binding::BindResponse, CommonError> {
    let query = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                    executor_processor::table.find(executor_processor_id)
                                             .select((executor_processor::id,
    ...
    let response: Result<service_binding::EncryptedBindResponse, CommonError> =
...
async fn activate_executor_row(conn: db::PoolConnection,
    ...
    // TODO:
    // Consider caching tokens to be used when collecting executor-events, and
    // health checks. This will avoid querying the database.
    // However, cached record operations cannot be placed in the context of the
    // operation db update token.
