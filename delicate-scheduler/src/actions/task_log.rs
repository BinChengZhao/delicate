...
pub(crate) fn route_config() -> Route {
    Route::new().at("/api/task_log/event_trigger", post(create_task_logs))
...
async fn create_task_logs(Json(events_collection): Json<delicate_utils_task_log::SignedExecutorEventCollection>,
                          pool: Data<&Arc<db::ConnectionPool>>)
                          -> impl IntoResponse {
    let r = async {
                debug!("Event collection - {:?}", &events_collection.event_collection);
...
async fn pre_create_task_logs(events_collection: delicate_utils_task_log::SignedExecutorEventCollection,
                              pool: Data<&Arc<db::ConnectionPool>>)
                              -> Result<usize, CommonError> {
    use delicate_utils_task_log::EventType;

    let executor_processor_id =
        (events_collection).event_collection
                           .events
                           .get(0)
                           .map(|e| e.executor_processor_id)
                           .ok_or_else(|| {
                               CommonError::DisPass(" `event_collection` is empty . ".into())
                           })?;
    ...
    events.into_iter().for_each(|e| match Into::<EventType>::into(e.event_type) {
...
async fn show_task_logs(Json(query_params): Json<model::QueryParamsTaskLog>,
                        pool: Data<&Arc<db::ConnectionPool>>)
                        -> impl IntoResponse {
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           ...
                           let task_logs = query_params.clone()
    ...
    Json(UnifiedResponseMessages::<PaginateData<model::FrontEndTaskLog>>::error())
}

#[handler]

async fn show_task_log_detail(Json(query_params): Json<model::RecordId>,
                              pool: Data<&Arc<db::ConnectionPool>>)
                              -> impl IntoResponse {
    ...
    if let Ok(conn) = pool.get() {
        let f_result = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                           let task_log_extend =
                               task_log_extend::table.find(query_params.record_id.0)
        ...
        let log_extend =
            f_result.map(|log_extend_result| {
...
async fn kill_task_instance(req: &Request,
...
fn batch_insert_task_logs(conn: &db::PoolConnection,
    ...
    if !new_task_logs.is_empty() {
        ...
        let tasks: HashMap<i64, model::task::SupplyTask> =
            model::TaskQueryBuilder::query_supply_task_log().filter(task::id.eq_any(&task_ids[..]))
        ...
        return diesel::insert_into(task_log::table).values(&new_task_logs[..]).execute(conn);
    }

    Ok(0)
}

fn batch_update_task_logs(conn: &db::PoolConnection,
    ...
    for supply_task_log in supply_task_logs.iter() {
        effect_num += diesel::update(&supply_task_log.0).set(&supply_task_log.0).execute(conn)?;
    }

    let supply_task_logs_extend: Vec<model::SupplyTaskLogExtend> =
        supply_task_logs.into_iter().map(|model::SupplyTaskLogTuple(_, t)| t).collect();

    diesel::insert_into(task_log_extend::table).values(&supply_task_logs_extend[..])
                                               .execute(conn)?;

    Ok(effect_num)
}

async fn kill_one_task_instance(req: &Request,
                                pool: Data<&Arc<db::ConnectionPool>>,
                                model::TaskRecord { task_id, record_id, executor_processor_id }: model::TaskRecord)
                                -> Result<(), CommonError> {
    ...
    let host = spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
                   let host =
                       task_log::table.find(&record_id.0)
                                      .filter(task_log::status.eq(state::task_log::State::Running
                                                                  as i16))
    ...
    let record = delicate_utils_task_log::CancelTaskRecord::default().set_task_id(task_id)
                                                                     .set_record_id(record_id.0)
                                                                     .set_time(get_timestamp())
                                                                     .sign(token.as_deref())?;

    request_client.post(url)
                  .json(&record)
                  .send()
                  .await?
...
#[handler]
async fn delete_task_log(req: &Request,
    ...
    if let Ok(conn) = pool.get() {
        return Json(Into::<UnifiedResponseMessages<()>>::into(pre_delete_task_log(delete_params,
                                                                                  conn).await));
    }
...
async fn pre_delete_task_log(delete_params: model::DeleteParamsTaskLog,
                             conn: db::PoolConnection)
                             -> Result<(), CommonError> {
    use db::schema::{task_log, task_log_extend};

    // Because `diesel` does not support join table deletion, so here is divided
    // into two steps to delete logs.

    // 1. query the primary key of task-log according to the given conditions, with
    // a single maximum limit of 524288 items.

    // 2. the primary key in batches of 2048 items and then start executing the
    // deletion, task-log and task-log-extend.

    spawn_blocking::<_, Result<_, diesel::result::Error>>(move || {
        let query_builder = model::TaskLogQueryBuilder::query_id_column();
        let task_log_ids = delete_params.query_filter(query_builder).load::<i64>(&conn)?;
