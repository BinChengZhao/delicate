...
fn main() {
    ...
    match database.as_str() {
        ...
        _ => {
            panic!("No reasonable `DATABASE` is set in .env, optional value is ('mysql' or 'postgres')")
        },
    }

    //  Authentication-Model, currently optional value `casbin` （optional
    // feature）.
    env::var("AUTH_MODEL").map(|a| match a.as_str() {
    ...
    env::var("APP_DEBUG").map(|a| match a.as_str() {
